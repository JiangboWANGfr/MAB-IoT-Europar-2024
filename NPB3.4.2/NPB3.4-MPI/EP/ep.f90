...
!   the "embarassingly parallel" benchmark.
    ...
    ! IOT1_core_number = get_cpu_cores()
    ! IOT2_core_number = get_cpu_cores()
    ! IOT3_core_number = get_cpu_cores()
    ! IOT4_core_number = get_cpu_cores()
    ! IOT1_core_number = 4
    ! IOT2_core_number = 4
    ! IOT3_core_number = 4
    ! IOT4_core_number = 4
    ...
    ! write(*, 889) cpu_cores
    ! 889  format('[DEBUG]: Detected CPU cores: ', I1)
...
!   divide the total number
    ...
    ! call  get_number_per_process_original(node,no_nodes,nn, no_large_nodes, np,np_add,k_offset)
    ! call get_number_per_process(node,no_nodes, nn,IOT1_ratio,IOT2_ratio, &
    ! IOT1_core_number, IOT2_core_number,k_offset, np)
    
    call computeRowsPerProcess(comm_solve,node, no_nodes,nn,IOT1_ratio, IOT2_ratio,IOT3_ratio, IOT4_ratio, &
    IOT1_core_number, IOT2_core_number, IOT3_core_number, IOT4_core_number, k_offset, np)
...
!       endif
    ...
    !   write(*, 891) no_nodes, np, no_large_nodes
        ! 891  format('[DEBUG]: no_nodes: ', I2, '   np: ', I4, '   no_large_nodes: ', I1)
...
!   have more numbers to generate than others
    ...
    !   endif
    !   print *,"[DEBUG]: Rank: ", node, " k_offset: ", k_offset
...
!        vectorizable. 
 ...
 150  continue
      ...
      call MPI_Allreduce(sx, x, 1, dp_type,MPI_SUM, comm_solve, ierr)
      sx = x(1)
      call MPI_Allreduce(sy, x, 1, dp_type,MPI_SUM, comm_solve, ierr)
      sy = x(1)
      call MPI_Allreduce(q, x, nq, dp_type,MPI_SUM, comm_solve, ierr)
 ...
 160  continue
      ...
      tm  = timer_read(1)
    !   write(*, 667) node, tm
    !     667 format('Rank: ', I2, '   Time: ', F10.3)
    !   write(*,777) node, k_offset, np, tm
    !     777 format("[DEBUG]:      Rank: ",I2,"          start_row: ",I5,"       num_rows: ",I5,"       time: ",F6.3)
     ...
     &                   MPI_MAX, comm_solve, ierr)
      ...
      if (node.eq.root) then
        ...
        ! set verifiedStr based on verified
        if (verified) then
            verifiedStr = 'SUCCESSFUL'
        else
            verifiedStr = 'UNSUCCESSFUL'
        end if
        ! write (6,11) class,mm, tm, verifiedStr , (i, q(i), i = 0, nq - 1)
        write (6,11) class,mm,nn, tm, verifiedStr
        11 format (//'EP Benchmark Results:'/&
                    & 'Class     =', a6/&
                    & 'N         =     2^',i2," = ",i4/&
                    & 'CPU Time  =',f10.3/&
                    & 'Verified  =',a17)
